    ...
    def train(self, features_df: pd.DataFrame) -> dict:
        ...
        Args:
            features_df: DataFrame with one row per employee.
                         Must contain a 'risk_zone' column with values
                         'GREEN', 'AMBER', or 'RED'.

        Returns:
            Metadata dict with accuracy, feature importance, etc.
        """
        if "risk_zone" not in features_df.columns:
            raise ValueError("features_df must contain 'risk_zone' column")

        y = features_df["risk_zone"].map(RISK_MAP).values
        X = self._prepare_features(features_df, fit=True)

        # ── Validate class balance before splitting ────────────────────────
        # build_features_batch aggregates to one row per employee, so a CSV
        # with many comment rows can still collapse to very few employees
        # per risk_zone. A stratified split requires >=2 members per class;
        # sklearn's raw error here is opaque ("least populated class has
        # only 1 member...") so we surface real counts instead.
        unique, counts = np.unique(y, return_counts=True)
        class_counts = {RISK_ZONES[int(u)]: int(c) for u, c in zip(unique, counts)}
        missing_zones = [z for z in RISK_ZONES if z not in class_counts]
        if missing_zones:
            raise ValueError(
                f"Training data has no employees labelled {', '.join(missing_zones)}. "
                f"Found: {class_counts}. Need at least one employee per zone "
                f"(GREEN, AMBER, RED) to train, after rows are aggregated by employee_id."
            )

        too_small = {z: c for z, c in class_counts.items() if c < 2}
        can_stratify = not too_small
        if too_small:
            logger.warning(
                f"[Train] Class(es) with fewer than 2 employees after aggregation: {too_small} "
                f"(full counts: {class_counts}). Falling back to a non-stratified split — "
                f"add more labelled employees per zone for a more reliable model."
            )
        ...
        try:
            ...
            # Last-resort fallback: still let training proceed without a
            # held-out test set rather than failing outright, since the
            # model itself doesn't require a split to fit.
            logger.warning(f"[Train] Split failed ({e}); training on full dataset without a held-out test set.")
        ...
        report = classification_report(
            y_test, y_pred,
            labels=list(range(len(RISK_ZONES))),
            target_names=RISK_ZONES,
            output_dict=True,
            zero_division=0,
        )
            ...
            for i in range(len(X)):
                ...
                for j in range(len(self.feature_columns)):
                    factor_list.append({
                        ...
                        "shap_value": round(float(np.asarray(row_shap[j]).reshape(-1)[0]), 4),
