...
Patch-target convention after the routers/ refactor:
  Module-level imports in each router file are the patch targets.
  e.g. `routers.surveys.analyze_batch` patches the name bound in surveys.py.
"""
...
def test_login_failure(client):
    with patch("routers.auth.db_authenticate_user", return_value=None):
...
def test_list_users(client, admin_headers):
    ...
    with patch("routers.users.db_list_users", return_value=mock_users):
...
def test_add_user(client, admin_headers):
    with patch("routers.users.db_create_user", return_value=True):
        response = client.post(
            "/users",
            json={"username": "new_mgr", "password": "password123",
...
def test_remove_user(client, admin_headers):
    with patch("routers.users.db_delete_user", return_value=True):
        response = client.delete("/users/new_mgr", headers=admin_headers)
        assert response.status_code == 200
        assert response.json()["status"] == "deleted"

    with patch("routers.users.db_delete_user", return_value=False):
...
@patch("routers.surveys.analyze_batch")
@patch("routers.surveys.db_insert_surveys")
def test_upload_surveys(mock_db_insert, mock_analyze_batch, client, admin_headers):
    mock_analyze_batch.return_value = [{"score": 0.5, "label": "positive", "probabilities": {}}]
...
@patch("routers.surveys.db_get_all_surveys")
@patch("routers.surveys.summarize_feedback")
...
@patch("routers.classification.db_get_all_surveys")
@patch("routers.classification.build_features_batch")
@patch("routers.classification.RAGClassifier")
@patch("routers.classification.db_save_classifications")
@patch("routers.classification.db_create_alert")
def test_classify_employees(
    mock_create_alert, mock_save_class, mock_clf_class,
    mock_build_features, mock_get_surveys, client, admin_headers,
):
...
@patch("routers.classification.db_get_latest_classifications")
...
@patch("routers.employees.db_get_employee_surveys")
...
@patch("routers.analytics.get_dashboard_kpis")
@patch("routers.analytics.db_get_zone_trend")
def test_analytics_dashboard(mock_trend, mock_kpis, client, admin_headers):
...
@patch("routers.analytics.db_get_alerts")
...
@patch("routers.analytics.db_acknowledge_alert")
