...
import {
  ...
  generateEmployeeRecommendation,
} from "@/lib/api"
import {
  Search, Play, RefreshCw, X, AlertTriangle,
  TrendingDown, TrendingUp, Minus, ChevronRight,
  Calendar, MessageSquare, Brain, ShieldAlert, CheckCircle, Sparkles,
} from "lucide-react"
import {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      toast.success(
        ...
        `${rec.priority.toUpperCase()} priority · view it in the Action Center → Recommendations tab`
      )
    } catch (e: any) {
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border flex-shrink-0"
          ...
          <div>
            ...
            {emp.department && <p className="text-[11px] text-muted mt-0.5">{emp.department}</p>}
          </div>
          ...
          {data && (
            <>
              <button
                ...
                disabled={generating}
                className="btn-violet btn-sm w-full justify-center"
              >
                {generating
                  ? <><RefreshCw className="w-3.5 h-3.5 animate-spin" />Generating recommendation…</>
          ...
          <div className="page-container animate-fade-up">
            ...
            <div className="bg-surface border border-border rounded-xl overflow-hidden" style={{ boxShadow: "var(--shadow-card)" }}>
              <table className="data-table">
                <thead>
                  <tr>
                    <th>Employee ID</th><th>Department</th><th>Risk Zone</th>
