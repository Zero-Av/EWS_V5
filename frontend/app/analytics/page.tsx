        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {loading ? Array.from({ length: 4 }).map((_, i) => <KpiCardSkeleton key={i} />) : (
            <>
              ...
              <KpiCard label="RED (RED)"    value={kpis?.zone_distribution?.RED ?? 0}   icon={AlertTriangle} iconColor="red"   valueColor={kpis?.zone_distribution?.RED > 0 ? "var(--red)" : undefined} />
              <KpiCard label="AMBER (AMBER)"     value={kpis?.zone_distribution?.AMBER ?? 0} icon={AlertTriangle} iconColor="amber" valueColor="var(--amber)" />
